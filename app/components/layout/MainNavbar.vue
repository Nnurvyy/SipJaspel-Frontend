<template>
  <div id="topbar">
    ...
    <div class="topbar-actions">
      <div class="period-badge">JAN 2026</div>
...
const { pageMap, unitData } = useJaspelStore();
...
const heading = computed(() => {
  ...
  if (routeName.startsWith('unit-id')) {
    ...
    return unitData[id]?.title || 'Unit Pelayanan';
  }
  return pageMap[routeName]?.heading || 'SIPJASPEL';
});

const sub = computed(() => {
  const routeName = route.name?.toString() || '';
  if (routeName === 'index') return 'Selamat datang di Sistem Informasi Jaspel Puskesmas';
  if (routeName.startsWith('unit-id')) return 'Distribusi Jaspel unit pelayanan — Januari 2026';
  return pageMap[routeName]?.sub || '';
});
</script>
