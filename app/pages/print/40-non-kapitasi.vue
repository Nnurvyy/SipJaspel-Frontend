<template>
  <div class="page">
    <div class="section-header">
      ...
      <p>Distribusi Jaspel Langsung dari Non Kapitasi — Januari 2026</p>
    </div>
    ...
    <div class="card" v-else>
      <div class="card-header">
        <div class="card-title">Distribusi 40% Non Kapitasi per Unit Pelayanan (2026-01)</div>
        <div class="card-subtitle">Semua nilai dalam Rupiah. Klik aksi edit untuk input manual per baris.</div>
      </div>
      <div class="table-scroll">
        <table id="tbl-print-40nk">
          <thead>
            <tr>
              <th>No</th>
              <th>Nama</th>
    ...
    <div v-if="isEditOpen" class="modal-overlay" @click.self="closeEdit">
      <div class="modal-card">
        <div class="modal-header">
          <h3>Edit Data 40% — {{ editForm.nama }}</h3>
          <button class="modal-close" @click="closeEdit">×</button>
        </div>
        <div class="modal-body">
          <p style="font-size:13px;color:var(--text-muted);margin:0 0 12px">Input nilai Jaspel per unit pelayanan (dalam Rupiah)</p>
          <div class="form-row">
...
// Initialize table rows from pegawai list, unit amounts are manual
const rows = ref<any[]>([]);
...
#tbl-print-40nk th { font-size: 11px; white-space: nowrap; }
</style>
